...
function confirmAndWrite(filePath: string, originalContent: string, newContent: string): Promise<void> {
    return new Promise(resolve => {
        ...
        const differences: string = diff.createPatch(path.basename(filePath), originalContent, newContent);
...
async function processFile(filePath: string, regex: RegExp): Promise<void> {
    try {
        ...
        const newContent: string = originalContent.replace(regex, (match: string, p1: string, p2: string) => {
...
async function processVueFile(filePath: string): Promise<void> {
    try {
        ...
        const updatedContent: string = originalContent.replace(scriptBlockRegex, (match: string, openTag: string, scriptContent: string, closeTag: string) => {
