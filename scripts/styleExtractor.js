...
function loadConfig() {
    ...
    if (fs.existsSync(configPath)) {
        try {
            ...
            console.error(`[ERROR] Failed to load or parse config file ${CONFIG.configFile}. Using default configuration. Error: ${error.message}`);
        }
    } else {
    ...
    try {
        ...
        if (nonDirectiveLines.length === 0 && trimmedContent.length > 0) {
            ...
            if (importRegexTest.test(trimmedContent)) {
                ...
                console.log(`[MIGRATE] Migrating @import to @use in directive-only block for: ${componentName}.vue`);
                ...
                let newDirectiveContent = trimmedContent.replace(importRegexReplace, (match, p1, p2, p3) => {
                ...
                console.log(`[REFAC] Migrated @import to @use in ${componentName}.vue (directive-only block).`);
                
                return; // Stop here, migration is complete.
                
            } else {
                // If it only contained @use (or no @import), skip.
                console.log(`[SKIP] Style block in ${componentName}.vue contains only @use/comments. Skipping extraction/migration.`);
                return;
            }
        }
...
function main() {
    ...
    if (!fs.existsSync(CONFIG.outputDirFull)) { // Using new camelCase name
    ...
    if (CONFIG.excludeFiles.length > 0) { // Using new camelCase name
